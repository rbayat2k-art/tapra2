...
// Shared by Sidebar (menu), App.tsx (tab guards) and action handlers (operations), so a
// second active role reliably unlocks menu + page + data + operations together.
...
// `required`" — encodes every special-case override that already existed in Sidebar.tsx's
// hasAccess (canCreateRequests, isDualRole for approval/payment permissions, the three-way
// manage_assigned_tasks check) on top of the plain effective-permission check. Both the
// Sidebar menu AND the App.tsx tab-render guard call this so a permission unlocks menu +
// page + data + operations consistently — never just the menu item.
