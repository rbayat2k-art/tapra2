...
const actionLabels: Record<CustomerImportAction, string> = {
  CREATE_NEW: 'ساخت Customer جدید', LINK_TO_EXISTING: 'اتصال به Customer موجود',
...
const reasonLabels: Record<string, string> = {
  ...
  existing_normalized_phone_match: 'شماره تلفن در Customer 360 وجود دارد',
  duplicate_normalized_phone_in_file: 'همین شماره در ردیف قبلی فایل است',
  existing_normalized_name_match: 'نام مشابه در Customer 360 وجود دارد',
...
export function CustomerImportView() {
  ...
  const [sourceName, setSourceName] = useState('ورودی آزمایشی Customer 360');
  ...
  const loadJobs = useCallback(async () => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : 'فهرست Importها دریافت نشد.'); }
  ...
  const openJob = async (jobId: string) => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : 'جزئیات Import دریافت نشد.'); }
  ...
  const upload = async (event: FormEvent) => {
    ...
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'فایل در staging ثبت نشد.'); }
    finally { setBusy(false); }
  };
  ...
  const mutate = async (operation: () => Promise<{ import: CustomerImportJob }>) => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : 'عملیات Import انجام نشد.'); }
    finally { setBusy(false); }
  };
  ...
  return <section className="max-w-7xl space-y-5 dir-rtl">
    ...
    <div className="grid gap-5 lg:grid-cols-[270px_minmax(0,1fr)]">
      ...
      </article> : <div className="rounded-2xl border border-dashed border-slate-700 p-12 text-center text-slate-500">یک Import را انتخاب کنید یا فایل جدیدی در staging قرار دهید.</div>}
    </div>
  </section>;
}
...
function ImportRecordCard({ record, job, disabled, onDecide }: {
  ...
  return <section className="rounded-2xl border border-slate-800 bg-slate-900 p-4">
    ...
    {!disabled && <div className="mt-3 flex flex-wrap gap-2">
      {record.classification !== 'INVALID' && <button type="button" onClick={() => onDecide(record, 'CREATE_NEW')} className="rounded-lg border border-slate-600 px-3 py-2 text-xs">Customer جدید</button>}
    ...
    {record.appliedCustomerId && <p className="mt-3 text-xs text-emerald-400">به Customer 360 منتقل شد · {record.appliedCustomerId}</p>}
  </section>;
}
