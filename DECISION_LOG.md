...
**Decision:** طراحی هدف پروفایل یکپارچهٔ مشتری، Merge/Split تحت اختیار مدیر داده، وضعیت‌های چندبُعدی، ادعای خرید، پروموشن نسخه‌دار، پرونده‌های اجرای مستقل خدمات، تغییر پروموشن پس از پرداخت ناقص، قاعدهٔ ارجاع لغو/شکایت/عودت به پس از فروش، سلسله‌مراتب اجرای خدمت، پیامک رویدادمحور، OTP و پرتال مشتری ثبت شد. مرجع جزئیات `docs/CUSTOMER_PROFILE_PROMOTION_AND_SERVICE_FULFILLMENT.md` است.

**Reason:** تصمیم‌ها طی چند جلسه و تصاویر CRM قبلی پراکنده بودند و خطر اشتباه گرفتن نقش/مجوز با ماژول عملیاتی وجود داشت.

**Impact:** اسناد روی شاخهٔ مستقل منتشر می‌شوند و `stable` تغییر نمی‌کند. پیاده‌سازی پروفایل نیز در شاخهٔ محلی جدا تا پایان تست کاربر Push نمی‌شود. گزارش‌ها باید وضعیت «پیاده‌شده»، «زیرساخت‌فقط»، «ناقص» و «برنامه‌ریزی‌شده» را جدا کنند.
