...
**Decision:**
اصلاحیه امنیتی Impersonation + زیرساخت کامل نقش‌های سازمان فروش + بازطراحی فلوی پرداخت عادی و افزودن پرداخت فوری (روی `feature/multi-role-permissions`، بدون merge به `stable`):

۱. **رفع دو باگ امنیتی از قبل موجود** (کشف‌شده حین بررسی کد پیش از پیاده‌سازی):
   - `LoginRegisterModal.tsx`: حذف بای‌پس رمز عبور جهانی (`cleanPass !== '123456' && cleanPass !== 'admin'`) — اکنون فقط `foundUser.password === cleanPass` پذیرفته می‌شود.
   - `storage.ts` `getCurrentUser()`: حذف fallback خودکار به `DEFAULT_USERS[0]` (ادمین ارشد) وقتی نشست معتبری در `localStorage` نبود — نبود نشست همیشه صفحه‌ی لاگین را نشان می‌دهد.
۲. **سخت‌سازی Impersonation** (حفظ کامل قابلیت، طبق دستور صریح کاربر): `SystemPermission` جدید `impersonate_users`؛ `handleImpersonateUser` در `App.tsx` روی `realActor = impersonatorAdmin || currentUser` چک می‌کند (نه `currentUser` لحظه‌ای) تا فراخوانی مستقیم handler هم مسدود شود؛ اینترفیس جدید `ImpersonationLogEntry` (کلید `IMPERSONATION_LOG`) شروع/پایان هر نشست را با هویت واقعی ادمین ثبت می‌کند؛ Logout کامل هم `currentUser` هم `impersonatorAdmin` (state + localStorage) را پاک و رکورد باز را می‌بندد؛ بنر «در حال مشاهده سیستم به‌جای کاربر...» با دکمه بازگشت همیشه‌قابل‌مشاهده.
۳. **۵ نقش سازمان فروش با شناسه‌ی فنی پایدار**: `role_salesperson`, `role_sales_supervisor`, `role_senior_sales_supervisor`, `role_sales_manager`, `role_sales_deputy` در `DEFAULT_ROLES` — عمداً بدون `manage_vendors`/`create_request`/`view_branch_requests`. ۲۰ مجوز ریزدانه‌ی جدید در `SystemPermission` (از `view_own_customers` تا `manage_sales_hierarchy`) + کاتالوگ آن‌ها در `ALL_PERMISSIONS` (`RolesAndPermissionsView.tsx`، دسته‌ی «ماژول فروش»). ۳ کاربر نمونه‌ی موجود (`user_sales_person_1/supervisor_1/manager_1`) با `roleId` صریح اصلاح شدند (به‌جای `role:'requestor'` که به‌اشتباه `manage_vendors` می‌داد)؛ ۲ کاربر نمونه‌ی جدید (`user_sales_senior_supervisor_1`, `user_sales_deputy_1`) اضافه و زنجیره‌ی ۵سطحی کامل چیده شد. `getVisibleCustomerIds` در `salesHierarchy.ts` بازنویسی شد تا عمق دید را از روی مجوز مؤثر (`view_own_customers`/`view_team_customers`/`view_descendant_customers`) تعیین کند، نه صرفاً «همیشه کل زیردرخت»؛ توابع جدید `getSalesSubordinateIds`, `getDirectSalesReportIds`, `canAssignLeadTo`.
۴. **مدل چندنقشی: Deny صریح**: `User.deniedPermissions?: SystemPermission[]` — در `getEffectiveUserPermissions` (`permissions.ts`) از اجتماع نقش‌ها/customPermissions کم می‌شود؛ Deny همیشه بر Allow اولویت دارد. UI ویرایش در `AdminPanel.tsx` (چک‌لیست Deny + پیش‌نمایش «کدام نقش هر پرمیشن را می‌دهد» با `getGrantingRoleId`).
۵. **دفاع سه‌لایه برای قفل دسترسی مالی**: تابع مشترک جدید `canAccessNavItem` (`permissions.ts`) منطق ویژه‌ی موجود (`canCreateRequests`, `isDualRole`, `manage_assigned_tasks`) را از `Sidebar.hasAccess` بیرون کشید تا هم `Sidebar.tsx` (منو) هم `App.tsx` (Tab Guard جدید — `useEffect` که با هر تغییر `activeTab`/مجوز، تب‌های غیرمجاز را به `dashboard` برمی‌گرداند) از همان منبع استفاده کنند؛ handlerهای عملیات هم از همان `effectivePermissions`/`hasPermission` استفاده می‌کنند. یک ناسازگاری موجود (`Sidebar.tsx` `canSeeVendorCategories` که فقط `customPermissions` را چک می‌کرد، نه مجوز مؤثر کامل) هم در همین گذر رفع شد.
۶. **فلوی پرداخت عادی**: باگ `canMarkPaid` (`RequestDetailModal.tsx`) که اجازه‌ی پرداخت درخواست `pending_approval` (هنوز تاییدنشده) را می‌داد، رفع شد — اکنون فقط `approved_pending_payment` + ارجاع اختصاصی (`currentApproverId === currentUser.id`). `ApprovalInboxView.tsx`: fallback نقش‌محور قدیمی (هر `treasury_executor` همه‌ی درخواست‌های `approved_pending_payment`/`pending_approval` را می‌بیند) حذف شد. دکمه‌ی جدید «ارجاع پرداخت» (فقط ادمین) مسئول پرداخت را تغییر می‌دهد. حذف کامل تصویر placeholder Unsplash که هنگام نبود فیش جایگزین می‌شد؛ اکنون `paidWithoutReceipt: true` + وضعیت شفاف در UI.
۷. **پرداخت فوری (جدید، غیرفعال به‌صورت پیش‌فرض)**: `SystemPermission` جدید `refer_for_emergency_payment`/`execute_emergency_payment`؛ `RequestStatus` مقدار جدید `emergency_pending_payment`؛ فیلدهای جدید روی `PaymentRequest` (`isEmergencyPayment`, `emergencyReason`, `emergencyReferredByUserId/Name`, `emergencyReferredAt`). نقش `role_emergency_payment_officer` تعریف شد ولی به هیچ کاربر نمونه‌ای assign نشد. مسیر دو-نفره با دلیل اجباری و منع خودارجاعی (لیست مقصد ارجاع فوری همیشه ارجاع‌دهنده را حذف می‌کند). بج «فوری» در `RequestTableView.tsx`/`ArchiveView.tsx`؛ زیرتب «گزارش پرداخت‌های فوری» در `AdminPanel.tsx`.
۸. **Audit Log با دامنه‌ی محدود و مستند**: فایل جدید `src/utils/auditLog.ts` (`logAudit`) + اینترفیس `AuditLogEntry` (کلید `AUDIT_LOG`) — فقط چرخه‌ی Impersonation و فلوی پرداخت عادی/فوری را پوشش می‌دهد (نه یک audit log سراسری کل سیستم)؛ هر رکورد `effectiveUserId` و در صورت Impersonation `impersonatorAdminId`/`effectiveRoleId` را ثبت می‌کند.

**Reason:**
کاربر صریحاً درخواست داد Impersonation حذف نشود بلکه سخت‌تر شود، زیرساخت کامل سازمان فروش (نه فقط دید ساده‌ی قبلی) در همین برنچ ساخته شود، دسترسی مالی هرگز به‌صورت ناخواسته از طریق نقش‌های فروش نشت نکند، مدل چندنقشی معنای Deny صریح داشته باشد، و دو شکاف واقعی گردش کار پرداخت (پرداخت درخواست تأییدنشده، و نبود مسیر پرداخت فوری کنترل‌شده) بسته شود — همه‌ی این‌ها با بررسی مستقیم کد قبل از پیاده‌سازی تایید شدند، نه فرض.

**Impact:**
هر ۱۴ سناریوی تست درخواستی کاربر (ورود امن ادمین به‌جای کاربر، منع Impersonation برای غیرادمین، بازگشت صحیح پس از Refresh، ثبت هویت واقعی در Audit Log، نبود دسترسی دفترچه برای نقش‌های فروش، اعطای انتخابی مجوز مالی، اجتماع مجوزهای دو نقش، اولویت Deny، منع پرداخت مستقیم تأییدنشده، ارجاع پرداخت توسط ادمین، پرداخت بدون فیش، پرداخت فوری مجاز، منع پرداخت فوری بدون مجوز، منع خودارجاعی) در بخش Verification همین commit تایید شدند. هیچ‌کدام از ۶ کاربر نمونه‌ی خزانه‌داری، `isDualRole`, `approvalChain`, `allowedApproverIds` یا نام/فیلد اینترفیس موجودی در `types.ts` تغییر یا rename نشدند — فقط موارد کاملاً جدید اضافه، و دو باگ امنیتی مستقل رفع شدند. تغییرات فقط روی `feature/multi-role-permissions` push شدند؛ `stable` دست‌نخورده ماند.
