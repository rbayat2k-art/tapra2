...
| مدیریت کامل Campaign/Promotion، Sales Invoice/Commission، Finance، Support، Organization UI و Communications | `Prototype-backed` تا vertical sliceهای بعدی |
...
- `npm test`: `426/426` موفق در `25` فایل، شامل PostgreSQL، Sales Lead isolation/policy/marketing linkage، migration populated database و تست‌های domain legacy.
- browser: Login، context selection/switching، Dashboard، Sales Lead/queue/assignment/call/relationship lock/marketing context، Finance vendor directory، Support، RBAC، Customer 360 و Import بررسی شدند.
