...
| Automated tests | `npm test` | `426` آزمون در `25` فایل: PostgreSQL Foundation/Customer/Import/Sales و unit testهای Sales/Finance/Support/RBAC بازیابی‌شده |
| Migration verification | اجرای test suite | ساخت schema از ابتدا تا `0010` و upgrade از database دارای Customer قبل از `0008` روی `tapra2_test` |
...
تست‌های PostgreSQL مسیر migrationهای `0001` تا `0010`، permission منفی Import/Sales، identity مشترک Workspace با relationship جدا در چند Company، tenant isolation، migration داده موجود، CSV staging/approval، Lead assignment/reassignment، failed/effective call، relationship lock، Campaign/Promotion linkage و snapshot، provenance، timeline و audit را پوشش می‌دهند. testهای بازیابی‌شده قواعد Dashboard profile، navigation/RBAC، Catalog، Sales Invoice/Coordination/Fulfillment، Support refund و storage compatibility را نیز اجرا می‌کنند.

آخرین اجرای ثبت‌شده `25` فایل و `426/426` test موفق داشت. build Web هشدار chunk بزرگ‌تر از `500 kB` دارد و code splitting یک کار آینده است.
