...
| Migration verification | اجرای test suite | ساخت schema تا `0017` از ابتدا و اجرای دقیق legacy Sales `0009/0010` با checksum رسمی، bridge، rerun و حفظ داده بدون reset |
...
آخرین اجرای کامل شامل `27/27` فایل و `473/473` test موفق بود. Browser validation این Run مدیریت حساب وصول، تغییر و بازگردانی policy تأیید سرپرست، نمایش ماسک‌شده حساب و Flow فارسی/RTL را با موفقیت بررسی کرد و console مرورگر error/warning نداشت. تست PostgreSQL، partial payment بدون release، exact full payment با release، correction مستقل، overpayment denial، RLS چندشرکتی، Audit و bridge دقیق legacy را اثبات کرد. build Web هشدار chunk بزرگ‌تر از `500 kB` دارد و code splitting یک کار آینده است.
