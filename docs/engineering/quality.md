...
| Automated tests | `npm test` | `425` آزمون در `25` فایل: PostgreSQL Foundation/Customer/Import/Sales و unit testهای Sales/Finance/Support/RBAC بازیابی‌شده |
| Migration verification | اجرای test suite | ساخت schema از ابتدا تا `0009` و upgrade از database دارای Customer قبل از `0008` روی `tapra2_test` |
...
تست‌های PostgreSQL مسیر migrationهای `0001` تا `0009`، permission منفی Import/Sales، identity مشترک Workspace با relationship جدا در چند Company، tenant isolation، migration داده موجود، CSV staging/approval، Lead assignment/reassignment، failed/effective call، relationship lock، provenance، timeline و audit را پوشش می‌دهند. testهای بازیابی‌شده قواعد Dashboard profile، navigation/RBAC، Catalog، Sales Invoice/Coordination/Fulfillment، Support refund و storage compatibility را نیز اجرا می‌کنند.

آخرین اجرای ثبت‌شده `25` فایل و `425/425` test موفق داشت. build Web هشدار chunk بزرگ‌تر از `500 kB` دارد و code splitting یک کار آینده است.

Browser validation دستی Sales شامل ایجاد Lead برای Customer 360، assignment، ورود فروشنده، مشاهده صف شخصی، تماس ناموفق بدون lock، تماس مؤثر با `until_reassigned` و بازتخصیص manager با reason بود؛ console مرورگر error/warning نداشت.
